...
def build(barks):
    ...
    out.append('            ""')
    out.append("")
    out.append("    # THE LINE AT A KNOWN POSITION, added 2026-08-30 so the caption can")
    out.append("    # print the words the player is actually hearing.")
    out.append("    #")
    out.append("    # GetBark above picks for itself, which was fine while the caption WAS")
    out.append("    # the announcer. Once the clips existed, the sound box picked one line")
    out.append("    # and the caption picked another, and the two disagreed on screen. Kai,")
    out.append("    # 2026-08-30: \"can we wrie totso the caption is following wahts being")
    out.append("    # aid\". So the announcer now picks a NUMBER, plays that numbered sound")
    out.append("    # box, and asks here for the line at the same number.")
    out.append("    #")
    out.append("    # THE ORDER OF THIS ARRAY IS THEREFORE LOAD-BEARING. It is the order of")
    out.append("    # barks.py, which is the order of the recording sheet, which is the")
    out.append("    # order the Audio Players must sit in inside their list in UEFN.")
    out.append("    GetBarkAt<public>(Trigger : string, Index : int):string =")
    out.append("        if (Found := Lines[Trigger], Pick := Found[Index]):")
    out.append("            Pick")
    out.append("        else:")
    out.append('            ""')
    out.append("")
